...
public class ProductPriceResourceIntegrationTest {
    ...
    void testGetProducts() {
        ...
        given().when().get("").then().statusCode(200).body("size()", is((int) productsCount))
                .body("[0].id", CoreMatchers.any(Integer.class)).body("[0].sku", is("00000001"))
                .body("[0].price", is(29.99f)).body("[1].id", CoreMatchers.any(Integer.class))
                .body("[1].sku", is("00000002")).body("[1].price", is(37.99f))
                .body("[2].id", CoreMatchers.any(Integer.class)).body("[2].sku", is("00000003"))
                .body("[2].price", is(1299.99f)).body("[3].id", CoreMatchers.any(Integer.class))
                .body("[3].sku", is("00000004")).body("[3].price", is(799.99f))
                .body("[4].id", CoreMatchers.any(Integer.class)).body("[4].sku", is("00000005"))
                .body("[4].price", is(59.99f)).body("[5].id", CoreMatchers.any(Integer.class))
                .body("[5].sku", is("00000006")).body("[5].price", is(150.00f))
                .body("[6].id", CoreMatchers.any(Integer.class)).body("[6].sku", is("00000007"))
                .body("[6].price", is(79.95f)).body("[7].id", CoreMatchers.any(Integer.class))
                .body("[7].sku", is("00000008")).body("[7].price", is(79.99f));
    ...
    void testGetProductById() {
        String responseBody = given().when().get("/201").then().statusCode(200).body("id", is(201))
                .body("sku", is("00000005")).body("price", is(59.99f)).extract().asString();
    ...
    void testCreateProduct() {
        CreateProductPriceDTO createProductDTO =
                new CreateProductPriceDTO("10000009", BigDecimal.valueOf(300.00));

        ProductPrice product = given().header("Content-type", "application/json")
                .body(createProductDTO).when().post("").then().statusCode(201)
                .body("id", is(CoreMatchers.any(Integer.class))).body("sku", is("10000009"))
                .body("price", is(300.00f)).extract().as(ProductPrice.class);
    ...
    void testUpdateProduct() {
        UpdateProductPriceDTO updateProductDTO =
                new UpdateProductPriceDTO(productToBeUpdated.getId(), BigDecimal.valueOf(350.00));
    ...
    void testPostWithEmptyBody() {
        ...
        verify(productRepository, times(0)).saveProduct(any(ProductPrice.class));
    }
    ...
    void testPatchWithEmptyBody() {
        ...
        verify(productRepository, times(0)).updateProduct(any(ProductPrice.class));
    }
}
